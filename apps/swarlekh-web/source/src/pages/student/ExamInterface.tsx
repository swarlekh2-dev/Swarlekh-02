...
export default function ExamInterface() {
  ...
  // The SpeechRecognition callbacks below are registered once per recording
  // session, so they close over whatever state existed at that moment. These
  // refs give those callbacks a live view of the current state instead.
  ...
  useEffect(() => { buffersRef.current = sentenceBuffers }, [sentenceBuffers])
  ...
  const getCurrentQuestion = (): Question | null =>
    (examRef.current ?? exam)?.questions?.[currentQRef.current] || null

  const getCurrentBuffer = () => buffersRef.current[getCurrentQuestion()?.id || ''] || []

  // Chrome's SpeechRecognition emits a *series* of final results that are
  // progressive refinements of the same utterance — "my", "my name",
  // "my name is", "my name is Prajakta". Appending each one produced the
  // "my. my name. my name is. my name is Prajakta" duplication. When the new
  // chunk simply extends the previous one, replace it instead of appending.
  const mergeSentence = (buffer: string[], incoming: string): string[] => {
    const clean = incoming.trim().replace(/\s+/g, ' ')
    if (!clean) return buffer
    if (buffer.length === 0) return [clean]
    const prev = buffer[buffer.length - 1]
    const a = prev.toLowerCase()
    const b = clean.toLowerCase()
    // New chunk is a longer version of the last one → it supersedes it.
    if (b.startsWith(a)) return [...buffer.slice(0, -1), clean]
    // Last chunk already contains this one → the engine repeated itself.
    if (a.startsWith(b)) return buffer
    // Genuinely a new sentence.
    return [...buffer, clean]
  }

  const updateAnswer = (qId: string, text: string, buffer: string[]) => {
    // Update the ref synchronously — the speech callbacks read it before
    // React has had a chance to flush the state update.
    buffersRef.current = { ...buffersRef.current, [qId]: buffer }
  ...
  const handleVoiceCommand = (transcript: string, qId: string): boolean => {
    ...
    if (t.includes('next question') || t.includes('पुढचा') || t.includes('next')) {
      ...
      if (q < ((examRef.current ?? exam)?.questions?.length || 0) - 1) {
        goToQuestion(q + 1)
        speak(`Moving to question ${q + 2}`)
    ...
    if (t.includes('previous') || t.includes('मागचा') || t.includes('go back')) {
      ...
      if (q > 0) {
        goToQuestion(q - 1)
        speak(`Going back to question ${q}`)
  ...
  const startRecording = () => {
    ...
    recognition.onstart = () => { isRecordingRef.current = true; setIsRecording(true); setInterimText('') }
    // Long dictation sessions end on their own every ~60s; restart so the
    // student can keep speaking. Reads the ref, not the stale state value.
    recognition.onend = () => { if (isRecordingRef.current) { try { recognition.start() } catch {} } }
    recognition.onerror = (e: any) => {
      // 'no-speech' and 'aborted' are normal during pauses — don't kill the session.
      if (e.error === 'aborted' || e.error === 'no-speech') return
      isRecordingRef.current = false
      setIsRecording(false)
      toast.error('Recording stopped. Tap mic to restart.')
    }
    recognition.onresult = (event: any) => {
      ...
      const spoken = final.trim().replace(/\s+/g, ' ')
      if (!spoken) return
      const qId = getCurrentQuestion()?.id
      if (!qId) return
      if (handleVoiceCommand(spoken, qId)) return
      const buf = mergeSentence(getCurrentBuffer(), spoken)
      // Nothing changed — the engine just repeated itself, stay quiet.
      if (buf === getCurrentBuffer()) { setInterimText(''); return }
      buffersRef.current = { ...buffersRef.current, [qId]: buf }
      updateAnswer(qId, buf.join('. '), buf)
      setInterimText('')
      // Read back only the newest sentence, not the whole answer.
      speak(buf[buf.length - 1])
    }
